...
# Mock modules before importing aggregation_service
...
def test_mock_omdb_limit():
    ...
    # We need to mock settings as well since it might try to load .env
    with patch('config.settings') as mock_settings:
        mock_settings.OMDB_API_KEY = 'mock_key'
        mock_settings.TMDB_API_KEY = 'mock_key'
        mock_settings.DATABASE_URL = 'sqlite:///:memory:'
        
        aggregator = DataAggregationService(db)
        
        # Mock TMDB detail call
        aggregator.tmdb = MagicMock()
